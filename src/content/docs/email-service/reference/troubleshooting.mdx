...
1. In **DNS** > **Records**, add the five Google Workspace `MX` records and a new `TXT` record on the root domain for domain verification. Refer to [Set up Google Workspace DNS records](/dns/manage-dns-records/how-to/set-up-google-workspace/) for the complete record values and the DKIM setup steps required to activate Google Workspace email. The Cloudflare Email Routing `MX` records remain active until the next step.
