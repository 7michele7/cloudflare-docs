...
2. Select your domain, then open **Settings** and turn off Email Routing. Cloudflare removes all routing-related DNS records it manages (`MX`, `SPF`, and `DKIM`), as described in [Remove a domain from Email Routing](/email-service/configuration/domains/#remove-a-domain-from-email-routing).
3. In **DNS** > **Records**, add all the DNS records required for Google Workspace: domain verification (`TXT`), `MX` records, `SPF`, and `DKIM`. Refer to [Set up Google Workspace DNS records](/dns/manage-dns-records/how-to/set-up-google-workspace/) for the complete record values and setup steps.
